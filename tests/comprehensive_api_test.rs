...
async fn create_comprehensive_test_app_state() -> AppState {
    ...
    let work_order_service =
        stateset_api::services::work_orders::WorkOrderService::new(db_arc.clone());
    ...
    AppState {
        ...
        redis: Arc::new(
            redis::Client::open(config.redis_url.clone()).unwrap_or_else(|_| {
...
async fn create_test_app() -> axum::Router {
    ...
    axum::Router::new()
        .nest(
            "/health",
            health::health_routes_with_state(state.db.clone()),
        )
        .nest(
            "/api/v1",
...
async fn test_error_handling() {
    ...
    let (status, body) = make_request(
        &app,
        Method::GET,
        &format!("/api/v1/orders/{}", fake_uuid),
        None,
    )
    .await;
    ...
    // Test invalid JSON
    let (status, body) =
        make_request(&app, Method::POST, "/api/v1/orders", Some(json!("invalid"))).await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
...
async fn test_pagination() {
    ...
    let (status, body) =
        make_request(&app, Method::GET, "/api/v1/orders?page=1&limit=10", None).await;
    assert_eq!(status, StatusCode::OK);
    assert!(body["data"]["items"].is_array());
...
async fn test_auth_endpoints() {
    ...
    let (status, body) =
        make_request(&app, Method::POST, "/api/v1/auth/login", Some(login_data)).await;
...
async fn test_fallback_handling() {
    ...
    let (status, body) =
        make_request(&app, Method::GET, "/api/v1/non-existent-endpoint", None).await;
    assert_eq!(status, StatusCode::NOT_FOUND);
...
async fn test_large_payload_handling() {
    ...
    let (status, body) =
        make_request(&app, Method::POST, "/api/v1/orders", Some(large_order)).await;
