...
pub trait MigratorTrait {
    ...
    async fn up(db: &DatabaseConnection, schema_manager: Option<&SchemaManager>) -> Result<(), DbErr> {
        let schema_manager = match schema_manager {
            Some(schema_manager) => schema_manager.clone(),
    ...
    async fn down(db: &DatabaseConnection, schema_manager: Option<&SchemaManager>) -> Result<(), DbErr> {
        let schema_manager = match schema_manager {
            Some(schema_manager) => schema_manager.clone(),
            None => SchemaManager::new(db),
        };
