...
lazy_static! {
    static ref WARRANTY_CREATIONS: IntCounter = 
        IntCounter::new("warranty_creations_total", "Total number of warranties created")
            .expect("metric can be created");

    static ref WARRANTY_CREATION_FAILURES: IntCounter = 
        IntCounter::new("warranty_creation_failures_total", "Total number of failed warranty creations")
            .expect("metric can be created");
}
...
impl Command for CreateWarrantyCommand {
    ...
    async fn execute(
        ...
        let warranty = warranty::ActiveModel {
            ...
            description: Set(Some(format!("Warranty for product {} with serial number {}", 
        ...
        event_sender.send(Event::WarrantyCreated(result.id)).await
            .map_err(|e| {
                WARRANTY_CREATION_FAILURES.inc();
