...
impl Command for CompleteWorkOrderCommand {
    ...
    async fn execute(&self, db_pool: Arc<DbPool>, event_sender: Arc<EventSender>) -> Result<Self::Result, ServiceError> {
        ...
        let updated_work_order = conn.transaction(|| {
            self.complete_work_order(&conn)
        }).map_err(|e| {
            error!("Transaction failed for completing Work Order ID {}: {}", self.work_order_id, e);
            e
        })?;
...
impl CompleteWorkOrderCommand {
    fn complete_work_order(&self, conn: &PgConnection) -> Result<WorkOrder, ServiceError> {
        diesel::update(work_orders::table.find(self.work_order_id))
            .set((
                work_orders::status.eq(WorkOrderStatus::Completed),
                work_orders::completed_at.eq(Utc::now()),
            ))
            .get_result::<WorkOrder>(conn)
            .map_err(|e| {
                error!("Failed to complete Work Order ID {}: {}", self.work_order_id, e);
    ...
    async fn log_and_trigger_event(&self, event_sender: Arc<EventSender>, work_order: &WorkOrder) -> Result<(), ServiceError> {
        ...
        event_sender.send(Event::WorkOrderCompleted(work_order.id))
            .await
            .map_err(|e| {
                error!("Failed to send WorkOrderCompleted event for Work Order ID {}: {}", work_order.id, e);
