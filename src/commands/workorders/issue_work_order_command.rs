...
impl Command for IssueWorkOrderCommand {
    ...
    async fn execute(
        ...
        let updated_work_order = db
            .transaction(|txn| Box::pin(async move { self.issue_work_order(txn).await }))
            .await
            .map_err(|e| {
                error!(
                    "Transaction failed for issuing Work Order ID {}: {}",
...
impl IssueWorkOrderCommand {
    ...
    async fn log_and_trigger_event(
        ...
        event_sender
            .send(Event::WorkOrderIssued(work_order.id))
            .await
            .map_err(|e| {
                error!(
                    "Failed to send WorkOrderIssued event for Work Order ID {}: {}",
                    work_order.id, e
                );
