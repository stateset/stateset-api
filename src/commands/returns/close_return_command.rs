        ...
        let result = db.transaction::<_, Self::Result, ServiceError>(|txn| {
            Box::pin(async move {
                ...
                Ok(CloseReturnResult {
                    ...
                    closed: true,
                    closed_at: updated_return.updated_at.to_rfc3339(),
...
impl CloseReturnCommand {
    ...
    async fn close_return(
        ...
        let mut updated_metadata = return_request.metadata.clone()
            .unwrap_or_else(|| serde_json::json!({}));
            
        if let Some(reason) = &self.reason {
            if let serde_json::Value::Object(ref mut map) = updated_metadata {
                map.insert("close_reason".to_string(), serde_json::Value::String(reason.clone()));
                map.insert("closed_at".to_string(), serde_json::Value::String(now.to_string()));
                if let Some(closed_by) = &self.closed_by {
                    map.insert("closed_by".to_string(), serde_json::Value::String(closed_by.clone()));
                }
            }
        }
        ...
        if let Some(metadata) = &self.metadata {
            if let (serde_json::Value::Object(ref mut map), serde_json::Value::Object(ref new_data)) = 
    ...
    async fn create_history_record(
        ...
        if let Some(reason) = &self.reason {
            history_metadata.insert("reason".to_string(), serde_json::Value::String(reason.clone()));
        }
