...
impl RefundReturnCommand {
    async fn refund_return(
        ...
        let refunded_return = return_request
            ...
            .await
            .map_err(|e| {
                let msg = format!("Failed to refund return ID {}: {}", self.return_id, e);
                error!("{}", msg);
                ServiceError::DatabaseError(msg)
            })?;
    ...
    async fn log_and_trigger_event(
        ...
        info!("Return refunded for return ID: {}. Amount: {}", self.return_id, self.refund_amount);
        event_sender
            .send(Event::ReturnRefunded(self.return_id))
            .await
            .map_err(|e| {
