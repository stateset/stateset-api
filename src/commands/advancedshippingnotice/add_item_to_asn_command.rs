...
lazy_static! {
    static ref ASN_ITEMS_ADDED: IntCounter = 
        IntCounter::new("asn_items_added_total", "Total number of items added to ASNs")
            .expect("metric can be created");

    static ref ASN_ITEM_ADD_FAILURES: IntCounter = 
        IntCounter::new("asn_item_add_failures_total", "Total number of failed item additions to ASNs")
            .expect("metric can be created");
}
...
impl AddItemToASNCommand {
    ...
    async fn add_item_to_asn(
        &self,
        db: &DatabaseConnection,
    ) -> Result<asn_item_entity::Model, ServiceError> {
    ...
    async fn log_and_trigger_event(
        ...
        saved_item: &asn_item_entity::Model,
    ) -> Result<(), ServiceError> {
        info!(
            asn_id = %self.asn_id,
        ...
        event_sender
            .send(Event::ASNItemAdded(self.asn_id, saved_item.id))
            .await
            .map_err(|e| {
                ASN_ITEM_ADD_FAILURES.inc();
