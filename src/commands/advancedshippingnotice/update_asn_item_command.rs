...
impl Command for UpdateASNItemsCommand {
    ...
    async fn execute(
        ...
        if !vec![ASNStatus::Draft.to_string(), ASNStatus::Submitted.to_string()]
            .contains(&current_asn.status) {
            return Err(ASNError::InvalidStatus(self.asn_id));
        }

        let updated_items = db.transaction::<_, Vec<asn_item_entity::Model>, ASNError>(|txn| {
            Box::pin(async move {
        ...
        event_sender
            .send(Event::ASNItemsUpdated(self.asn_id, updated_items.iter().map(|i| i.id).collect()))
            .await?;
