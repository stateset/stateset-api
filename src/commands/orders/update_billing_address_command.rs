...
lazy_static! {
    static ref BILLING_ADDRESS_UPDATES: IntCounter =
        IntCounter::new("billing_address_updates_total", "Total number of billing address updates")
            .expect("metric can be created");

    static ref BILLING_ADDRESS_UPDATE_FAILURES: IntCounter =
        IntCounter::new("billing_address_update_failures_total", "Total number of failed billing address updates")
            .expect("metric can be created");
}
...
impl UpdateBillingAddressCommand {
    async fn update_billing_address(&self, db: &DatabaseConnection) -> Result<order_entity::Model, ServiceError> {
        ...
        order_active_model
            ...
            .map_err(|e| {
                BILLING_ADDRESS_UPDATE_FAILURES.inc();
                let msg = format!("Failed to update billing address for order ID {}: {}", self.order_id, e);
                error!("{}", msg);
                ServiceError::DatabaseError(msg)
            })
    }
    ...
    async fn log_and_trigger_event(&self, event_sender: Arc<EventSender>, _updated_order: &order_entity::Model) -> Result<(), ServiceError> {
        event_sender
            .send(Event::BillingAddressUpdated(self.order_id))
            .await
            .map_err(|e| {
                BILLING_ADDRESS_UPDATE_FAILURES.inc();
                error!("Failed to send BillingAddressUpdated event for order ID {}: {}", self.order_id, e);
