...
impl Command for HoldOrderCommand {
    ...
    async fn execute(&self, db_pool: Arc<DbPool>, event_sender: Arc<EventSender>) -> Result<Self::Result, OrderError> {
        ...
        let updated_order = hold_order_in_db(&db, self.order_id, &self.reason, self.version).await?;
...
async fn hold_order_in_db(db: &DatabaseConnection, order_id: i32, reason: &str, version: i32) -> Result<order_entity::Model, OrderError> {
