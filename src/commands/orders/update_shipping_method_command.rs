...
lazy_static! {
    static ref SHIPPING_METHOD_UPDATES: IntCounter =
        IntCounter::new("shipping_method_updates_total", "Total number of shipping method updates")
            .expect("metric can be created");

    static ref SHIPPING_METHOD_UPDATE_FAILURES: IntCounter =
        IntCounter::new("shipping_method_update_failures_total", "Total number of failed shipping method updates")
            .expect("metric can be created");
}
...
impl UpdateShippingMethodCommand {
    async fn update_shipping_method(&self, db: &DatabaseConnection) -> Result<order_entity::Model, ServiceError> {
        ...
        order_active_model
            ...
            .map_err(|e| {
                SHIPPING_METHOD_UPDATE_FAILURES.inc();
                let msg = format!("Failed to update shipping method for order ID {}: {}", self.order_id, e);
                error!("{}", msg);
                ServiceError::DatabaseError(msg)
            })
    }
    ...
    async fn log_and_trigger_event(&self, event_sender: Arc<EventSender>, _updated_order: &order_entity::Model) -> Result<(), ServiceError> {
        event_sender
            ...
            .map_err(|e| {
                SHIPPING_METHOD_UPDATE_FAILURES.inc();
                error!("Failed to send ShippingMethodUpdated event for order ID {}: {}", self.order_id, e);
