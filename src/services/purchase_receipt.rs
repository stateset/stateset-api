...
impl PurchaseReceiptService {
    ...
    pub async fn receive_po_line(
        ...
        let header = ReceiptHeaderEntity::find_by_id(shipment_header_id)
            ...
            .ok_or_else(|| ServiceError::NotFound(format!("Receipt header {} not found", shipment_header_id)))?;
        ...
        let total_received: Decimal = previous_receipts.iter()
        ...
        if let Some(sender) = &self.event_sender {
            let _ = sender.send(Event::PurchaseOrderReceived {
                po_line_id,
                item_id,
        ...
        info!("PO line {} received: {} units of item {} at location {}", 
    ...
    pub async fn receive_multiple_lines(
        ...
        for line in lines {
            let received = self.receive_po_line(
                shipment_header_id,
                line.po_header_id,
                line.po_line_id,
    ...
    pub async fn return_to_vendor(
        ...
        let receipt_line = ReceiptLineEntity::find_by_id(receipt_line_id)
            ...
            .await
            .map_err(|e| ServiceError::DatabaseError(e))?
            .ok_or_else(|| ServiceError::NotFound(format!("Receipt line {} not found", receipt_line_id)))?;

        let item_id = receipt_line.item_id
            .ok_or_else(|| ServiceError::InvalidOperation("Receipt line has no item".to_string()))?;
        
        let received_quantity = receipt_line.quantity_received.unwrap_or(Decimal::ZERO);
        
        if return_quantity > received_quantity {
            return Err(ServiceError::InvalidOperation(
                format!("Cannot return more than received. Received: {}, Trying to return: {}",
        ...
        // Send event
        if let Some(sender) = &self.event_sender {
            let _ = sender.send(Event::PurchaseOrderReturned {
                receipt_line_id,
                item_id,
        ...
        info!("Returned {} units of item {} to vendor from receipt line {}", 
    ...
    pub async fn get_po_receipt_status(
        ...
        for line in po_lines {
            ...
            let line_received: Decimal = receipts.iter()
                .map(|r| r.quantity_received.unwrap_or(Decimal::ZERO))
                .sum();
