...
impl WorkOrderService {
    ...
    // ... (other work order-related methods)
}

pub async fn create_work_order(pool: &DbPool, new_work_order: NewWorkOrder, created_by: i32) -> Result<WorkOrder, ServiceError> {
    let conn = pool.get()?;
    ...
    notify_new_work_order(&work_order, created_by).await?;

    Ok(work_order)
}

pub async fn get_work_order(pool: &DbPool, id: i32) -> Result<WorkOrder, ServiceError> {
    let conn = pool.get()?;
...
pub async fn update_work_order(pool: &DbPool, id: i32, updated_work_order: WorkOrder) -> Result<WorkOrder, ServiceError> {
    let conn = pool.get()?;
...
pub async fn delete_work_order(pool: &DbPool, id: i32) -> Result<(), ServiceError> {
    let conn = pool.get()?;
...
pub async fn list_work_orders(pool: &DbPool, pagination: PaginationParams) -> Result<Vec<WorkOrder>, ServiceError> {
    let conn = pool.get()?;
...
pub async fn search_work_orders(pool: &DbPool, search: WorkOrderSearchParams) -> Result<Vec<WorkOrder>, ServiceError> {
    let conn = pool.get()?;
...
pub async fn assign_work_order(pool: &DbPool, id: i32, user_id: i32) -> Result<WorkOrder, ServiceError> {
    let conn = pool.get()?;
    let work_order = diesel::update(work_orders::table)
        .filter(work_orders::id.eq(id))
        .set((
    ...
    notify_work_order_assignment(&work_order).await?;

    Ok(work_order)
}

pub async fn complete_work_order(pool: &DbPool)
