    ...
    pub async fn adjust_for_sales_order(
        ...
        if order_lines.is_empty() {
            return Err(ServiceError::NotFound(format!(
                "No order lines found for order {}", order_id
            )));
        }
        ...
        for receipt_line in receipt_lines {
            let po_line = PurchaseOrderLine::find()
                ...
                .ok_or_else(|| ServiceError::NotFound(format!(
                    "PO line {} not found", receipt_line.po_line_id
                )))?;

            let result = self.receive_inventory_for_po_line(
                db, 
                &po_line, 
    ...
    async fn allocate_inventory_for_order_line(
        ...
        db.transaction::<_, InventoryAdjustmentResult, ServiceError>(|txn| {
            Box::pin(async move {
                ...
                active_inventory.quantity_allocated = Set(inventory.quantity_allocated + required_qty);
                active_inventory.quantity_available = Set(inventory.quantity_available - required_qty);
        ...
        db.transaction::<_, InventoryAdjustmentResult, ServiceError>(|txn| {
            Box::pin(async move {
                ...
                let transaction = inventory_transaction::ActiveModel {
                    ...
                    notes: Set(Some(format!("Shipped for order line {}", order_line.line_id))),
    ...
    async fn deallocate_inventory_for_order_line(
        ...
        db.transaction::<_, InventoryAdjustmentResult, ServiceError>(|txn| {
            Box::pin(async move {
                ...
                // Update inventory balance
                let mut active_inventory: inventory_balance::ActiveModel = inventory.clone().into();
                active_inventory.quantity_allocated = Set(inventory.quantity_allocated - deallocate_qty);
                active_inventory.quantity_available = Set(inventory.quantity_available + deallocate_qty);
                active_inventory.updated_at = Set(Utc::now().into());
                ...
                let transaction = inventory_transaction::ActiveModel {
                    ...
                    notes: Set(Some(format!("Deallocated for cancelled order line {}", order_line.line_id))),
                    created_by: Set(Uuid::new_v4()),
                    created_at: Set(Utc::now()),
                };
    ...
    async fn return_inventory_for_order_line(
        ...
        db.transaction::<_, InventoryAdjustmentResult, ServiceError>(|txn| {
            Box::pin(async move {
                ...
                active_inventory.quantity_available = Set(inventory.quantity_available + return_qty);
                active_inventory.updated_at = Set(Utc::now().into());
                ...
                let transaction = inventory_transaction::ActiveModel {
                    ...
                    notes: Set(Some(format!("Returned from order line {}", order_line.line_id))),
                    created_by: Set(Uuid::new_v4()),
                    created_at: Set(Utc::now()),
                };
            ...
            Box::pin(async move {
                ...
                let (updated_inventory, was_created) = match inventory {
                    Some(inv) => {
                        ...
                        let mut active_inventory: inventory_balance::ActiveModel = inv.clone().into();
                        active_inventory.quantity_on_hand = Set(inv.quantity_on_hand + quantity_received);
                        active_inventory.quantity_available = Set(inv.quantity_available + quantity_received);
                        active_inventory.updated_at = Set(Utc::now().into());
                ...
                let transaction = inventory_transaction::ActiveModel {
                    ...
                    notes: Set(Some(format!("Received from PO line {}", po_line.po_line_id))),
                    created_by: Set(Uuid::new_v4()),
                    created_at: Set(Utc::now()),
                };
