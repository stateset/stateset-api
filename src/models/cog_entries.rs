...
pub struct Model {
    ...
    #[validate(length(min = 1, max = 100, message = "Product must be between 1-100 characters"))]
    pub product: String,
...
impl ActiveModelBehavior for ActiveModel {
    ...
    fn before_save(mut self, insert: bool) -> Result<Self, DbErr> {
        ...
        if insert {
            ...
            if self.is_return.as_ref().is_none() {
                self.is_return = Set(Some(false));
            }
        }
...
impl Model {
    ...
    pub fn calculate_gross_margin_percentage(&mut self) -> Result<(), CogsError> {
        match (self.gross_profit, self.gross_sales) {
            ...
            (None, _) => Err(CogsError::Calculation("Gross profit not calculated".to_string())),
            (_, None) => Err(CogsError::Calculation("Gross sales not set".to_string())),
            (_, Some(sales)) if sales.is_zero() => Err(CogsError::Calculation("Sales value is zero".to_string())),
            _ => Err(CogsError::Internal("Unexpected error in margin calculation".to_string())),
        }
    }
    ...
    pub fn calculate_cogs_percentage(&mut self) -> Result<(), CogsError> {
        match self.gross_sales {
            ...
            None => Err(CogsError::Calculation("Gross sales not set".to_string())),
            Some(sales) if sales.is_zero() => Err(CogsError::Calculation("Sales value is zero".to_string())),
            _ => Err(CogsError::Internal("Unexpected error in COGS percentage calculation".to_string())),
        }
    }
