...
impl Model {
    ...
    pub async fn save(
        ...
        if self.id == 0 {  // Only for new records
            if let Some(machine) = crate::models::machine::Entity::find_by_id(self.machine_id).one(db).await? {
                ...
                if let Some(installation_date) = machine_model.installation_date {
                    if let Some(maintenance_schedule_type) = machine_model.maintenance_schedule_type {
                ...
                let machine_active_model: crate::models::machine::ActiveModel = machine_model.into();
