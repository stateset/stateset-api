...
impl ActiveModelBehavior for ActiveModel {
    ...
    fn before_save(mut self, insert: bool) -> Result<Self, DbErr> {
        ...
        if insert {
            ...
            // Set default values for new shipments
            if self.status.as_ref().is_none() {
                self.status = Set(ShipmentStatus::Processing);
            }
            
            if self.is_signature_required.as_ref().is_none() {
                self.is_signature_required = Set(false);
            }
        }
...
impl Model {
    ...
    pub fn mark_as_shipped(
        ...
        if self.status != ShipmentStatus::Processing && self.status != ShipmentStatus::ReadyToShip {
            return Err(ShipmentError::InvalidOperation(
                format!("Cannot mark shipment as shipped from {} status", self.status)
            ));
        }
    ...
    pub fn is_delivery_late(&self) -> bool {
        match (self.status, self.estimated_delivery) {
            ...
            (_, Some(estimated)) => Utc::now() > estimated && self.status != ShipmentStatus::Delivered,
            _ => false,
        }
    }
}