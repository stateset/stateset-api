...
pub struct Model {
    ...
    #[validate(length(min = 1, max = 50, message = "ASN number must be between 1 and 50 characters"))]
...
impl Model {
    ...
    pub fn mark_in_transit(&mut self) -> Result<(), ValidationError> {
        if self.status != ASNStatus::Submitted {
            return Err(ValidationError::new("ASN must be in Submitted status to mark as in transit"));
        }
        
        if self.shipping_date.is_none() || self.carrier_type.is_none() || self.tracking_number.is_none() {
            return Err(ValidationError::new("Shipping information must be provided before marking ASN as in transit"));
        }
    ...
    pub fn mark_delivered(&mut self) -> Result<(), ValidationError> {
        if self.status != ASNStatus::InTransit {
            return Err(ValidationError::new("ASN must be in InTransit status to mark as delivered"));
        }
    ...
    pub fn place_on_hold(&mut self) -> Result<(), ValidationError> {
        if self.status == ASNStatus::Completed || self.status == ASNStatus::Cancelled {
            return Err(ValidationError::new("Cannot place completed or cancelled ASN on hold"));
        }
    ...
    pub fn release_from_hold(&mut self, new_status: ASNStatus) -> Result<(), ValidationError> {
        ...
        if new_status == ASNStatus::OnHold || new_status == ASNStatus::Completed {
            return Err(ValidationError::new("Invalid status transition from OnHold"));
        }
    ...
    pub fn cancel(&mut self) -> Result<(), ValidationError> {
        if self.status == ASNStatus::Completed || self.status == ASNStatus::Delivered {
            return Err(ValidationError::new("Cannot cancel completed or delivered ASN"));
        }
