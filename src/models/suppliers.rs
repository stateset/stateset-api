...
pub struct Model {
    ...
    #[validate(length(min = 1, max = 50, message = "City must be between 1 and 50 characters"))]
    pub city: String,
    
    #[validate(length(min = 1, max = 50, message = "State/province must be between 1 and 50 characters"))]
...
fn validate_birthdate(date: &Option<NaiveDate>) -> Result<(), ValidationError> {
    if let Some(birthdate) = date {
        let now = Utc::now().date_naive();
        
        // Must be at least 18 years old
        let min_age_date = NaiveDate::from_ymd_opt(
            now.year() - 18,
            now.month(),
            now.day().min(birthdate.day()),
        ).unwrap_or(now);
        
        if birthdate > &min_age_date {
            return Err(ValidationError::new("Supplier must be at least 18 years old"));
        }
        ...
        if birthdate < &max_age_date {
            return Err(ValidationError::new("Birthdate is too far in the past"));
        }
    }
...
fn validate_optional_phone(phone: &Option<String>) -> Result<(), ValidationError> {
    if let Some(phone_str) = phone {
        let validator = PhoneValidator::new();
        if !validator.is_valid(phone_str) {
            return Err(ValidationError::new("Invalid secondary phone number format"));
        }
    }
    Ok(())
}
...
impl ActiveModelBehavior for ActiveModel {
    ...
    fn before_save(mut self, insert: bool) -> Result<Self, DbErr> {
        ...
        if insert {
            ...
            self.total_orders = Set(0);
            
            // Set default values
            if self.status.is_none() {
                self.status = Set(SupplierStatus::Active);
            }
            
            if self.rating.is_none() {
                self.rating = Set(SupplierRating::Unrated);
            }
            
            if self.loyalty_points.is_none() {
                self.loyalty_points = Set(0);
            }
            
            if self.is_international.is_none() {
...
impl Model {
    ...
    pub fn add_tags(
        ...
        let current_tags: Vec<String> = self.tags.clone()
    ...
    pub fn remove_tags(
        ...
        if let Some(tags_str) = &self.tags {
            let current_tags: Vec<String> = tags_str
                .split(',')
    ...
    pub fn record_order(
        ...
        if let Some(days) = fulfillment_days {
            ...
            let new_avg = ((current_avg * (self.total_orders as f32 - 1.0)) + days) / self.total_orders as f32;
