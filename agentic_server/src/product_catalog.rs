...
impl ProductCatalogService {
    ...
    pub fn get_product(&self, product_id: &str) -> Result<Product, ServiceError> {
        ...
        products.get(product_id)
            ...
            .ok_or_else(|| ServiceError::NotFound(
                format!("Product {} not found or inactive", product_id)
            ))
    }
    ...
    pub fn reserve_inventory(
        ...
        debug!("Reserved {} units of {} for session {}", quantity, product_id, session_id);
        Ok(())
    }
    ...
    pub fn commit_inventory(&self, session_id: &str) -> Result<(), ServiceError> {
        ...
        let session_reservations: Vec<_> = reservations.iter()
        ...
        for reservation in &session_reservations {
            if let Some(product) = products.get_mut(&reservation.product_id) {
                ...
                info!("Committed {} units of {} (remaining: {})", 
...
mod tests {
    ...
    fn test_commit_inventory() {
        ...
        // Reserve and commit
        catalog.reserve_inventory("test", 10, "session_commit").unwrap();
